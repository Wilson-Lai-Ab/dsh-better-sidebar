...
describe('built-in tab registrations', () => {
  ...
  it('editor, diff and git-log are hidden from the + menu (opened by file-open / git view)', () => {
    const { service } = setup()
    expect(service.getTabs().filter(t => t.hidden).map(t => t.id).sort()).toEqual(['diff', 'editor', 'git-log'])
  })
