...
import { createSidebarStore, allLeaves, dockTabToCenter, makeDefaultState, openDiffTab, sanitizeState } from '../src/client/state.ts'
...
describe('service.openTab across the two panels', () => {
  ...
  it('reopening a file already on the conversation header focuses that tab', () => {
    const store = createSidebarStore()
    const service = createBetterSidebarService(store)
    service.registerTab({
      id: 'editor',
    ...
    store.setSession('s1')
    const tab = { id: 'editor:/ws/TimeUtils.java', type: 'editor', title: 'TimeUtils.java', path: '/ws/TimeUtils.java' }
    store.reduce(s => dockTabToCenter(s, 'seed', tab.id, tab))
    store.reduce(s => ({ ...s, centerActive: null, panelOpen: false }))
    service.openTab({ type: 'editor', title: 'TimeUtils.java', path: tab.path, id: tab.id })
    const state = store.getSnapshot().state!
    expect(state.centerTabs.map(item => item.id)).toEqual([tab.id])
    expect(state.centerActive).toBe(tab.id)
    expect(state.panelOpen).toBe(false)
    expect(allLeaves(state.splits).flatMap(leaf => leaf.tabs).some(item => item.id === tab.id)).toBe(false)
  })
...
describe('openFile (v0.12.0)', () => {
  ...
  it('lands on the conversation header (file previews dock there by default)', () => {
    ...
    expect(state.centerTabs.map(t => t.path)).toEqual([
      '/p/TimeUtils.java',
      '/p/ThreadPoolConfiguration.java',
    ])
    expect(allLeaves(state.splits).flatMap(l => l.tabs).some(t => t.path === '/p/ThreadPoolConfiguration.java')).toBe(false)
  })
})
