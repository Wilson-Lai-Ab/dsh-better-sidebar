...
describe('openFile (v0.12.0)', () => {
  ...
  it('stashes a reveal range before opening when reveal.line is set', async () => {
    const { takeReveal } = await import('../src/client/editor-reveal.ts')
    takeReveal('/p/src/main.ts')
    const store = createSidebarStore()
    const service = createBetterSidebarService(store)
    service.registerTab({ id: 'editor', title: () => 'Editor', component: () => null })
    store.setSession('s1')
    service.openFile({ sessionId: 's1' }, '/p/src/main.ts', undefined, {
      line: 12,
      endLine: 14,
      snippet: 'foo',
    })
    expect(takeReveal('/p/src/main.ts')).toEqual({ start: 12, end: 14, selected: 'foo' })
  })

  it('does not stash reveal when the fourth argument is omitted', async () => {
    const { takeReveal } = await import('../src/client/editor-reveal.ts')
    const store = createSidebarStore()
    const service = createBetterSidebarService(store)
    service.registerTab({ id: 'editor', title: () => 'Editor', component: () => null })
    store.setSession('s1')
    service.openFile({ sessionId: 's1' }, '/p/src/other.ts')
    expect(takeReveal('/p/src/other.ts')).toBeUndefined()
  })
})
