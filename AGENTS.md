...
| `git-log` | -1 | 否（按 id 去重） | 是 | Git 历史（底部面板，由 Git 面板「在底部打开」触发） |
...
- **活动栏宽度**：`--dsh-sidebar-rail-width`（活动栏横杆宽度，插件局部变量，缺省 44px，无硬编码颜色；皮肤可覆写以改活动栏粗细。消费点：`src/client/sidebar.module.css` 的 `.activityBar`）。
...
- **`src/client/builtins/`**：8 个内置 tab（explorer/git/subagent/terminal/browser/editor/diff/git-log）+ 6 个内置 viewer（image/pdf/markdown/html/code/binary-download）的注册代码（tabs.tsx / viewers.tsx / index.ts；Office 预览已迁至推荐插件，见 plugins-viewers.ts）
