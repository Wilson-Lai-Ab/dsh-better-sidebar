window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function ancestorDirsOf(cwd, path) {
		...
		*/
		function openTerminalInBottom(state, tab) {
			if (allLeaves(state.splits).concat(allLeaves(state.bottomSplits)).flatMap((leaf) => leaf.tabs).concat(state.centerTabs).filter((candidate) => candidate.type === "terminal" && !isAgentTabId(candidate.id)).length >= 3) return state;
		...
		* and squeeze the conversation to zero height. */
		...
		function expandExplorerToPath(state, cwd, path) {
		...
		function collapseAllExplorer(state) {
		...
		*/
		...
		var init_locales = __esmMin((() => {
			zh = {
				...
				edit: "编辑",
				refresh: "刷新",
				locateInExplorer: "定位到当前文件",
				collapseExplorer: "收起全部",
				save: "保存",
				saved: "已保存",
				unsaved: "未保存",
				saveFailed: "保存失败",
				...
				rename: "重命名",
				openGitHere: "在 Git 中打开",
				openInBrowser: "用浏览器打开",
				openInSystemBrowser: "系统浏览器",
				openInPluginBrowser: "插件浏览器",
				renameFailed: "重命名失败",
				settingsNav: "侧边卡片",
			...
			en = {
				...
				edit: "Edit",
				refresh: "Refresh",
				locateInExplorer: "Locate current file",
				collapseExplorer: "Collapse all",
				save: "Save",
				saved: "Saved",
				unsaved: "Unsaved",
				...
				rename: "Rename",
				openGitHere: "Open in Git",
				openInBrowser: "Open in Browser",
				openInSystemBrowser: "System Browser",
				openInPluginBrowser: "Sidebar Browser",
		...
		var init_sidebar_module_css = __esmMin((() => {
			...
			sidebar_module_css_default = {
				...
				"centerPreviewBodyActive": "X5YCQG_centerPreviewBodyActive",
				...
				"explorerHeaderActions": "X5YCQG_explorerHeaderActions",
				...
				"centerPreviewBodyInactive": "X5YCQG_centerPreviewBodyInactive",
				...
				"editorMdFallback": "X5YCQG_editorMdFallback",
				...
				"centerPreviewHosted": "X5YCQG_centerPreviewHosted",
				...
				"explorerRowActive": "X5YCQG_explorerRowActive",
				...
				"explorerRename": "X5YCQG_explorerRename",
				...
				"centerPreviewBodies": "X5YCQG_centerPreviewBodies",
				...
				"centerPreviewHidden": "X5YCQG_centerPreviewHidden",
		...
		function createBetterSidebarService(store) {
			...
			const openTab = (seed, scope) => {
				...
				const reducer = (state) => {
					...
					let landed = next;
					if (isCreation && (seed.url !== void 0 || seed.path !== void 0)) landed = patchTab(next, tab.id, {
		...
		*/
		...
		//#endregion
		//#region src/client/git-focus.ts
		let pending$1;
		const listeners$1 = /* @__PURE__ */ new Set();
		function requestGitFocus(focus) {
			pending$1 = focus;
			for (const listener of listeners$1) listener();
		}
		function takeGitFocus() {
			const focus = pending$1;
			pending$1 = void 0;
			return focus;
		}
		function subscribeGitFocus(listener) {
		...
		//#endregion
		//#region src/client/explorer/git-focus.ts
		/**
		* Pick which git work tree owns an explorer path, then the repo-relative
		* directory the Git panel should expand to.
		*/
		function gitFocusOf(path, isDir, repos) {
			const abs = path.replace(/\\/g, "/");
			const root = [...repos].map((repo) => repo.root.replace(/\\/g, "/").replace(/\/+$/, "")).filter((candidate) => abs === candidate || abs.startsWith(`${candidate}/`)).sort((a, b) => b.length - a.length)[0];
			...
			return {
				repo: root,
				dir: isDir ? rel : rel.includes("/") ? rel.slice(0, rel.lastIndexOf("/")) : ""
			};
		}
		//#endregion
		...
		*/
		function wrapOpenWorkspacePath(session, deps) {
			...
			const wrapped = (request, signal) => {
				if (deps.takeoverEnabled()) {
					...
					if (sessionId !== void 0 && request.path.length > 0) {
		...
		*/
		function registerOpenPathInterception(ctx, store) {
			...
			const restorePath = typeof ctx.workspaces?.openPath === "function" ? wrapOpenPath(ctx.workspaces, deps) : () => {};
			let session;
			try {
				const value = typeof ctx.get === "function" ? ctx.get("remote.session") : void 0;
				if (value !== void 0 && typeof value.openWorkspacePath === "function") session = value;
			} catch {
				session = void 0;
			}
			const restoreWorkspace = session !== void 0 ? wrapOpenWorkspacePath(session, deps) : () => {};
			return () => {
				restorePath();
				restoreWorkspace();
			};
		...
		const api = {
			...
			fsRename: (scope, from, to) => call("fs.rename", scopePayload(scope, {
				from,
				to
			})),
			fsReveal: (scope, path) => call("fs.reveal", scopePayload(scope, { path })),
			fsOpenInBrowser: (scope, path) => call("fs.openInBrowser", scopePayload(scope, { path })),
		...
		*/
		...
		function paintAncestors(map, abs, kind, root) {
		...
		function gitKindByPath(status) {
			...
			for (const entry of status.entries) {
				...
				paintAncestors(map, joinAbs(root, rel), kind, root);
				map.set(rel, stronger(map.get(rel), kind));
			}
			return map;
		}
		function sessionGitKind(kind) {
		...
		*/
		function sessionKindByPath(cwd, edits) {
			const map = /* @__PURE__ */ new Map();
			if (cwd === void 0 || cwd === "") return map;
		...
		*/
		function explorerKindOf(path, session, git, isDir = false) {
		...
		//#endregion
		//#region src/client/lh-pending.ts
		function sessionEditsFromLhPending(records) {
			const out = [];
			for (const record of records) {
				if (record.source !== void 0 && record.source !== "agent") continue;
				if (record.decision !== void 0 && record.decision !== "pending") continue;
		...
		*/
		...
		const IconLocateOutline16 = ({ size = 16, className }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
			width: size,
			height: size,
			className,
			viewBox: "0 0 16 16",
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
				...
				d: "M8 1.5v2.25M8 12.25V14.5M1.5 8h2.25M12.25 8H14.5",
				stroke: "currentColor",
				strokeWidth: "1.5",
				strokeLinecap: "round"
			})]
		...
		/** Collapse every expanded explorer directory. */
		const IconCollapseAllOutline16 = ({ size = 16, className }) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
			width: size,
			height: size,
			className,
			viewBox: "0 0 16 16",
			...
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
				d: "M3.5 7 8 3.5 12.5 7M3.5 12.5 8 9 12.5 12.5",
				stroke: "currentColor",
				strokeWidth: "1.5",
				strokeLinecap: "round",
				strokeLinejoin: "round"
		...
		//#endregion
		//#region src/client/explorer/reveal.ts
		init_state();
		...
		function filePathOf(tab) {
			...
			if (tab.type === "diff") {
				...
				if (tab.diff?.kind === "commit" && tab.diff.path !== void 0 && isWorkspaceFilePath(tab.diff.path)) return tab.diff.path;
			}
			if (typeof tab.path === "string" && isWorkspaceFilePath(tab.path)) return tab.path;
		}
		function previewFilePathOf(state, cwd) {
			const resolve = (path) => resolveSidebarPath(cwd, path);
			if (state.centerActive !== null) {
				const path = filePathOf(state.centerTabs.find((item) => item.id === state.centerActive));
				if (path !== void 0) return resolve(path);
			}
			const leaves = allLeaves(state.splits).concat(allLeaves(state.bottomSplits));
			const preferred = state.activePane === null ? void 0 : leaves.find((item) => item.id === state.activePane);
			const ordered = preferred === void 0 ? leaves : [preferred, ...leaves.filter((leaf) => leaf !== preferred)];
			for (const leaf of ordered) {
				const active = filePathOf(leaf.tabs.find((item) => item.id === leaf.active));
				if (active !== void 0) return resolve(active);
			}
			for (const leaf of ordered) for (const tab of leaf.tabs) {
				const path = filePathOf(tab);
				if (path !== void 0) return resolve(path);
			}
		}
		//#endregion
		//#region src/client/explorer/explorer-refresh.ts
		/**
		...
		*/
		const EXPLORER_POLL_MS = 2e3;
		/** Workspace root plus expanded folders, de-duplicating the root. */
		function visibleExplorerDirs(cwd, expanded) {
			if (cwd === void 0 || cwd === "") return [];
			const dirs = [cwd];
			for (const dir of expanded) if (dir !== "" && dir !== cwd) dirs.push(dir);
			return dirs;
		}
		/** True when both listings name the same entry paths in the same order. */
		function listingsEqual(previous, next) {
			if (previous === void 0) return false;
			if (previous.length !== next.length) return false;
			for (let i = 0; i < previous.length; i += 1) if (previous[i].path !== next[i].path) return false;
			return true;
		}
		//#endregion
		//#region src/client/explorer/plugin-browser.ts
		/**
		...
		*/
		function pluginBrowserHref(input) {
			if (input.isDir) return void 0;
			const origin = input.origin.replace(/\/+$/, "");
			const ext = input.path.split(/[\\/]/).pop()?.split(".").pop()?.toLowerCase();
			if (ext === "html" || ext === "htm") return `${origin}${encodeHtmlUrl(input.sessionId, input.path)}`;
			const params = new URLSearchParams({
				sessionId: input.sessionId,
				path: input.path
			});
			if (input.cwd !== void 0 && input.cwd !== "") params.set("cwd", input.cwd);
			return `${origin}/sidebar/file?${params.toString()}`;
		}
		//#endregion
		//#region src/client/explorer/row-menu.ts
		/** Ordered menu ids for one explorer row (separators omitted). */
		function explorerRowMenuIds(options) {
		...
		function terminalCwdOf(path, isDir) {
		...
		*/
		function siblingPathOf(path, nextName) {
		...
		*/
		...
		function ExplorerView(props) {
			const { ctx, sessionId, cwd, store, expanded, onToggle, onOpenFile, onOpenFileAbove, onReferenceFile, onOpenTerminal, onOpenGit, onOpenPluginBrowser } = props;
			...
			const [lhEdits, setLhEdits] = (0, react.useState)([]);
			const sessionKinds = (0, react.useMemo)(() => sessionKindByPath(cwd, lhEdits), [cwd, lhEdits]);
			(0, react.useEffect)(() => {
				...
				let cancelled = false;
				const load = () => {
					face.listReview(sessionId, cwd).then((result) => {
			...
			const rowKind = (path, isDir) => explorerKindOf(path, sessionKinds, gitKinds, isDir);
			...
			const [locatePath, setLocatePath] = (0, react.useState)(null);
			const snapshot = (0, react.useSyncExternalStore)(store?.subscribe ?? (() => () => {}), () => store?.getSnapshot().state, () => store?.getSnapshot().state);
			const previewPath = snapshot === void 0 ? void 0 : previewFilePathOf(snapshot, cwd);
			const canLocate = cwd !== void 0 && previewPath !== void 0 && ancestorDirsOf(cwd, previewPath) !== null;
			...
			(0, react.useEffect)(() => {
				...
				let cancelled = false;
				const poll = () => {
					for (const dir of dirs) api.fsTree({
						sessionId,
						cwd
					}, dir).then((listing) => {
						if (cancelled) return;
			...
			const commitRename = (from, nextName) => {
			...
			const rowName = (path, name, kindClass) => {
				if (renaming?.path === path) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
					className: sidebar_module_css_default.explorerRename,
					...
					onChange: (event) => {
						setRenaming((current) => current === null ? current : {
							...current,
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: clsx(sidebar_module_css_default.explorerName, kindClass),
					children: name
				});
			};
			const root = cwd;
				...
				return (level.entries ?? []).map((entry) => {
					if (entry.isDir) {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							className: clsx(sidebar_module_css_default.explorerRow, sidebar_module_css_default.explorerDir, entry.hidden && sidebar_module_css_default.explorerHidden, sameFsPath(locatePath ?? void 0, entry.path) && sidebar_module_css_default.explorerRowActive),
							style: { paddingLeft: depth * 22 + 6 },
							onDragStart: (event) => {
							...
							onClick: () => {
								if (renaming?.path !== entry.path) onToggle(entry.path);
							},
							onKeyDown: (event) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						draggable: true,
						"data-explorer-path": entry.path,
						className: clsx(sidebar_module_css_default.explorerRow, entry.hidden && sidebar_module_css_default.explorerHidden, (sameFsPath(previewPath, entry.path) || sameFsPath(locatePath ?? void 0, entry.path)) && sidebar_module_css_default.explorerRowActive),
						style: { paddingLeft: depth * 22 + 6 },
						title: entry.path,
						onDragStart: (event) => {
							startFileDrag(event, entry.path);
						...
						onClick: () => {
							if (renaming?.path !== entry.path) onOpenFile(entry.path);
						},
						onDoubleClick: (event) => {
				...
				children: [
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									disabled: !canLocate,
									onClick: () => {
										if (store === void 0 || cwd === void 0 || previewPath === void 0) return;
										setQuery("");
										store.reduce((s) => expandExplorerToPath(s, cwd, previewPath));
										setLocatePath(previewPath);
									},
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconLocateOutline16, { size: 14 })
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: sidebar_module_css_default.iconButton,
									...
									onClick: () => {
										store?.reduce((s) => collapseAllExplorer(s));
									},
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconCollapseAllOutline16, { size: 14 })
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: sidebar_module_css_default.iconButton,
						...
						children: root === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: [
								...
								hits?.map((hit) => {
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										role: "button",
										tabIndex: 0,
										className: clsx(sidebar_module_css_default.explorerRow, sidebar_module_css_default.explorerFindRow, sameFsPath(previewPath, hit.path) && sidebar_module_css_default.explorerRowActive),
										title: hit.rel,
										onClick: () => {
											onOpenFile(hit.path);
										},
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: clsx(sidebar_module_css_default.explorerName, classOfKind(rowKind(root, true))),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
						...
						items: rowMenu === null ? [] : explorerRowMenuIds({
							isDir: rowMenu.isDir,
							isRoot: cwd !== void 0 && rowMenu.path === cwd,
							inGit: gitFocusOf(rowMenu.path, rowMenu.isDir, repos) !== void 0
						}).flatMap((id) => {
							...
							const row = {
								...
								terminal: {
									...
									icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconTerminalOutline16, { size: 14 })
								},
								browser: {
									id,
									label: t("openInBrowser"),
									icon: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconGlobeOutline16, { size: 14 }),
									submenu: [{
										id: "browser-system",
										label: t("openInSystemBrowser")
									}, ...pluginHref === void 0 ? [] : [{
										id: "browser-plugin",
										label: t("openInPluginBrowser")
									}]]
								},
								rename: {
									id,
						...
						onSelect: (id) => {
							...
							if (id === "browser-plugin") {
								...
								if (href !== void 0) onOpenPluginBrowser?.(href);
								return;
							}
							if (id === "rename") {
								setRenameError(null);
		...
		//#endregion
		//#region src/client/editor-view-mode.ts
		const remembered$1 = /* @__PURE__ */ new Map();
		function viewModeKey(sessionId, path) {
			return `${sessionId}\n${path}`;
		}
		/** Closing the file tab (not hiding it behind 对话) drops the choice. */
		function forgetViewMode(sessionId, path) {
			remembered$1.delete(viewModeKey(sessionId, path));
		}
		//#endregion
		//#region src/client/preview-scroll.ts
		const remembered = /* @__PURE__ */ new Map();
		function keyOf(sessionId, path) {
			return `${sessionId}\n${path}`;
		}
		function forgetPreviewScroll(sessionId, path) {
		...
		*/
		function collapsedDirsForFocus(nodes, focusDir) {
			if (focusDir === "") return /* @__PURE__ */ new Set();
			const collapsed = /* @__PURE__ */ new Set();
			const walk = (items) => {
				for (const node of items) {
					if (node.kind !== "dir") continue;
					if (!(node.key === focusDir || focusDir.startsWith(`${node.key}/`) || node.key.startsWith(`${focusDir}/`))) collapsed.add(node.key);
					walk(node.children);
				}
			};
			walk(nodes);
		...
		*/
		...
		function GitPathTree(props) {
			...
			const [collapsed, setCollapsed] = (0, react.useState)(() => focusDir === void 0 ? /* @__PURE__ */ new Set() : collapsedDirsForFocus(nodes, focusDir));
			(0, react.useEffect)(() => {
				if (focusDir === void 0) return;
		...
		*  get a strikethrough so the removal reads at a glance). */
		function nameClassName(entry) {
			...
			if (badgeOf(entry) === "D") classes.push(sidebar_module_css_default.gitDeletedText);
		...
		*  letters set ('MM') lands in BOTH staged and modified. */
		...
		function GitView(props) {
			...
			const [focusDir, setFocusDir] = (0, react.useState)(cached?.focusDir);
			const repoRootRef = (0, react.useRef)(repoRoot);
			const groupByRef = (0, react.useRef)(groupBy);
			const refreshRef = (0, react.useRef)(async () => {});
			...
			*  changes ('MM') appears in BOTH staged and modified. */
			...
			(0, react.useEffect)(() => {
				const apply = (focus) => {
					...
					if (focus.repo !== repoRootRef.current) refreshRef.current(focus.repo);
				};
				const pending = takeGitFocus();
				if (pending !== void 0) apply(pending);
				return subscribeGitFocus(() => {
					const next = takeGitFocus();
					if (next !== void 0) apply(next);
				});
			}, [scope.sessionId, scope.cwd]);
		...
		*/
		...
		//#endregion
		//#region src/client/selection-quote-position.ts
		/** Place the selection action without entering the bottom composer area. */
		function selectionPopupPosition(rect, options) {
			...
			const top = below + popupHeight <= limit ? below : Math.max(0, Math.min(above, limit - popupHeight));
			return {
				...
				},
				{
					...
					component: ({ ctx, store, scope, expanded, onToggleDir, onReferenceFile }) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ExplorerView, {
						...
						onOpenTerminal: (dir) => {
							...
							store.reduce((s) => {
								const next = openTerminalInBottom(s, {
						...
						onOpenPluginBrowser: (href) => {
							...
							let title = t("browser");
							try {
								title = new URL(href).hostname || title;
							} catch {}
							ctx.betterSidebar?.openTab({
								type: "browser",
								url: href,
		...
		*/
		function previewOverlayTop(header, tablist, fallback = 48, origin) {
			...
			if (origin === void 0) return top;
			return Math.max(0, top - origin.top);
		}
		const COMPOSER_STACK_SELECTORS = ["[data-composer-seat]", "[data-composer-card]"];
		/**
		...
		*/
		function bottomDockTop(root) {
			...
			return tops.length === 0 ? void 0 : Math.min(...tops);
		}
		/** Conversation column that owns the session header — overlay host. */
		function previewOverlayBottom(options) {
			const { hostBottom, dockTop, panelInset, gap } = options;
			const dockInset = dockTop === void 0 ? 0 : Math.max(0, hostBottom - dockTop + gap);
			return Math.max(panelInset, dockInset);
		}
		function conversationPreviewHost(root = document) {
			const slot = root.querySelector("[data-slot=\"conversation.session.header\"]");
			const parent = slot === null ? null : slot.parentElement;
			return parent == null ? null : parent;
		}
		//#endregion
		//#region src/client/center-preview-mount.ts
		/**
		...
		*/
		function resolveCenterShown(tabIds, activeId, lastId) {
			if (activeId !== null && tabIds.includes(activeId)) return {
				shownId: activeId,
				chatting: false,
				lastId: activeId
			};
		...
		function isCenterBodyShown(tabId, shownId) {
		...
		*/
		/** Conversation column that owns the session header — overlay host. */
		function useConversationPreviewHost() {
			const [host, setHost] = (0, react.useState)(() => conversationPreviewHost());
			(0, react.useEffect)(() => {
				const check = () => {
		...
		function CenterPreview(props) {
			...
			const [hostedInset, setHostedInset] = (0, react.useState)({
				top: 0,
				bottom: 0
			});
			(0, react.useEffect)(() => {
				...
				const measure = () => {
					const header = host.querySelector("[data-slot=\"conversation.session.header\"] header") ?? host.querySelector("header");
					const tablist = header instanceof HTMLElement ? header.querySelector("[role=\"tablist\"]") : null;
					const origin = host.getBoundingClientRect();
					const nextTop = previewOverlayTop(header instanceof HTMLElement ? header.getBoundingClientRect() : null, tablist instanceof HTMLElement ? tablist.getBoundingClientRect() : null, 0, origin);
					...
					setHostedInset((prev) => prev.top === nextTop && prev.bottom === nextBottom ? prev : {
						top: nextTop,
						bottom: nextBottom
					});
				};
				measure();
				const Observer = typeof ResizeObserver === "function" ? ResizeObserver : void 0;
				const observer = Observer === void 0 ? void 0 : new Observer(measure);
			...
			const lastShownId = (0, react.useRef)(null);
			const shown = resolveCenterShown(state.centerTabs.map((tab) => tab.id), activeId, lastShownId.current);
			...
			const overlay = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: clsx(sidebar_module_css_default.centerPreview, hosted && sidebar_module_css_default.centerPreviewHosted, chatting && sidebar_module_css_default.centerPreviewHidden),
				style: hosted ? {
					top: hostedInset.top,
					bottom: 0
				} : {
				...
				children: [!hostHeaderPresent && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					className: sidebar_module_css_default.centerPreviewBodies,
					style: hosted ? { marginBottom: hostedInset.bottom } : void 0,
					children: state.centerTabs.map((candidate) => {
						const shownBody = isCenterBodyShown(candidate.id, shown.shownId);
						return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: clsx(sidebar_module_css_default.centerPreviewBody, shownBody ? sidebar_module_css_default.centerPreviewBodyActive : sidebar_module_css_default.centerPreviewBodyInactive),
			...
			return hosted ? (0, react_dom.createPortal)(overlay, host) : overlay;
		}
		//#endregion
