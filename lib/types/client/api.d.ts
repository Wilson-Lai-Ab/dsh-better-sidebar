...
export declare const api: {
    ...
    fsFind: (scope: SessionScope, query: string, signal?: AbortSignal) => Promise<{
        hits: FsFindHit[];
    }>;
    ...
    gitRepos: (scope: SessionScope, signal?: AbortSignal) => Promise<{
        repos: GitRepoInfo[];
    }>;
    ...
    gitStageTracked: (scope: SessionScope) => Promise<{
        ok: true;
    }>;
    /** Stage the given untracked paths in one batch. */
    gitStageUntracked: (scope: SessionScope, paths: string[]) => Promise<{
        ok: true;
    }>;
