...
const PrefsSchema = z.object({
	...
	centerTabMax: z.number().step(1).min(1).max(100).default(20),
	reviewDoneSessionLimit: z.number().step(1).min(1).max(50).default(30),
...
//#endregion
//#region src/explorer/fs-tree.ts
/**
...
function shouldSkipFindDir(name) {
	return name.startsWith(".") || SKIP_DIRS.has(name);
}
function normalizeRel$1(rel) {
	return rel.replace(/\\/g, "/");
}
function scoreAgainst(query, text, indexOffset) {
...
*/
function scoreFileNameMatch(query, rel) {
	const q = query.trim();
	if (q === "") return null;
	const path = normalizeRel$1(rel);
	if (q.includes("/") || q.includes("\\")) return scoreAgainst(q.replace(/\\/g, "/"), path, 0);
	const baseStart = path.lastIndexOf("/") + 1;
	const match = scoreAgainst(q, path.slice(baseStart), baseStart);
	if (match === null) return null;
	return {
		score: match.score + 24,
		indices: match.indices
	};
}
const FIND_MAX_MS = 1500;
const FIND_MAX_VISITED = 8e3;
function normalizeRel(rel) {
	return rel.replace(/\\/g, "/");
}
async function findFiles(root, query, options = {}) {
	const q = query.trim();
	if (q === "") return [];
...
//#endregion
//#region src/explorer/fs-reveal.ts
/**
...
*/
function revealCommand(platform, path, isDir) {
	...
	return {
		cmd: "xdg-open",
		args: [isDir ? path : dirname(path)]
	};
}
//#endregion
//#region src/explorer/fs-open-browser.ts
/**
* The spawn argv that opens `path` with the OS default application.
* @param platform - process.platform (injectable for tests).
* @param path - absolute file or directory.
*/
function openInBrowserCommand(platform, path) {
	if (platform === "darwin") return {
		cmd: "open",
		args: [path]
	};
...
//#endregion
//#region src/explorer/fs-rename.ts
/**
...
*/
const BAD_NAMES = /* @__PURE__ */ new Set([
	"",
	".",
	".."
]);
/** True when `name` is a single path segment (no slash, not `.` / `..`). */
function isRenameName(name) {
...
*/
function fencedRename(cwd, fromRaw, toRaw) {
	const from = requireAbsolute(fromRaw);
	const to = requireAbsolute(toRaw);
	if (!isWithin(cwd, from) || !isWithin(cwd, to)) throw new SidebarError("forbidden", "path is outside the session workspace", 403);
	if (from === cwd || to === cwd) throw new SidebarError("forbidden", "cannot rename the workspace root", 403);
	if (dirname(from) !== dirname(to)) throw new SidebarError("bad-request", "rename must stay in the same directory");
	if (!isRenameName(basename(to))) throw new SidebarError("bad-request", `invalid name "${basename(to)}"`);
	return {
		from,
		to
	};
...
//#endregion
//#region src/html-scroll-bridge.ts
/**
...
* HTML preview offset.
*/
const HTML_SCROLL_SOURCE = "dsh-better-sidebar:html-scroll";
const MARK = "data-dsh-html-scroll";
/** Inline listener injected into previewed HTML documents. */
function htmlScrollBridgeScript() {
...
function injectHtmlScrollBridge(html) {
...
const REPO_WALK_SKIP = /* @__PURE__ */ new Set([
	"node_modules",
	".git",
	"dist",
	"lib",
...
//#endregion
//#region src/review/review-disk.ts
/**
...
function reviewFilePath(root, cwd, sessionId) {
	const project = cwd === void 0 || cwd === "" ? join(root, "_no-cwd") : join(root, projectKey(cwd));
	return join(project, encodeSessionSegment(sessionId), REVIEW_FILE);
}
async function readReviewDocument(path) {
	try {
...
async function writeReviewDocument(path, doc) {
...
function buildApi(ctx, ptyManager, agentPtyRegistry, resolved, getSettings) {
	...
	/** Git work-tree cwd: honor `payload.repo` when it sits inside the session workspace. */
	const gitCwdOf = (payload) => {
		const { sessionId, cwd } = cwdOf(payload);
		const record = payload;
		const rawRepo = typeof record?.repo === "string" && record.repo !== "" ? record.repo : void 0;
		if (rawRepo === void 0) return {
			sessionId,
			cwd
		};
		let repo;
		try {
			repo = requireAbsolute(rawRepo);
		} catch {
			throw new SidebarError("bad-request", `invalid git repository "${rawRepo}"`);
		}
		if (!isWithin(cwd, repo)) throw new SidebarError("forbidden", `git repository "${repo}" is outside the session workspace`, 403);
		return {
			sessionId,
			cwd: repo
		};
	...
	return {
		...
		"fs.find": async (payload) => {
			const { cwd } = cwdOf(payload);
			const record = payload;
			return { hits: await findFiles(cwd, typeof record.query === "string" ? record.query : "", { limit: 50 }) };
		},
		...
		"fs.rename": async (payload) => {
			...
			try {
				...
				if (error.code !== "ENOENT") throw new SidebarError("fs-error", `cannot rename "${from}": ${error instanceof Error ? error.message : String(error)}`, 400);
			}
			try {
		...
		"fs.openInBrowser": async (payload) => {
			const { cwd } = cwdOf(payload);
			const path = requireAbsolute(requireString(payload, "path"));
			if (!isWithin(cwd, path)) throw new SidebarError("forbidden", `path is outside the session workspace`, 403);
			try {
				await stat(path);
			} catch (error) {
		...
		"git.repos": async (payload) => {
			const { cwd } = cwdOf(payload);
			return { repos: await listRepos(cwd) };
		},
		"git.status": async (payload) => {
			const { cwd } = gitCwdOf(payload);
			return status(cwd);
		},
		...
		"review.put": async (payload) => {
			const { sessionId, cwd } = cwdOf(payload);
			const document = parseReviewDocument(payload.document);
			await writeReviewDocument(reviewFilePath(defaultSessionsRoot(), cwd, sessionId), document);
			return document;
		},
		"pty.close": (payload) => {
			const sessionId = requireString(payload, "sessionId");
...
async function attachTerminal(ctx, ptyManager, agentPtyRegistry, ws, req, resolved) {
	try {
		...
		const sessionCwd = sessionCwdOf(ctx, sessionId, url.searchParams.get("cwd") ?? void 0);
